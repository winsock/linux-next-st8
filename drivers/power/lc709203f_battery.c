...
{
    struct lc709203f_chip *chip = container_of(psy, struct lc709203f_chip, battery);
    int temperature;
    int curr_ma;
    int ret = 0;
...
skip_thermistor_config:
    ...
    struct power_supply_desc *chip_desc;
    chip_desc = devm_kzalloc(&client->dev, sizeof(*chip_desc), GFP_KERNEL);
    if (!chip_desc)
        return -ENOMEM;
    
    chip_desc->name		= "battery";
    chip_desc->type		= POWER_SUPPLY_TYPE_BATTERY;
    chip_desc->get_property	= lc709203f_get_property;
    chip_desc->properties	= lc709203f_battery_props;
    chip_desc->num_properties	= ARRAY_SIZE(lc709203f_battery_props);
    chip->battery.desc = chip_desc;
    ...
    if (!chip->pdata->support_battery_current)
        chip_desc->num_properties--;
    
    ret = power_supply_register(&client->dev, chip->battery.desc, &chip->battery);
    if (ret) {
