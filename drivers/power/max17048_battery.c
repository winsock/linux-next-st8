                          ...
                          const struct i2c_device_id *id)
{
    ...
    chip_desc = devm_kzalloc(&chip->client->dev, sizeof(*chip_desc), GFP_KERNEL);
    if (!chip_desc)
        return -ENOMEM;
    
    chip_desc->name		= "battery";
    chip_desc->type		= POWER_SUPPLY_TYPE_BATTERY;
    chip_desc->get_property	= max17048_get_property;
    chip_desc->properties	= max17048_battery_props;
    chip_desc->num_properties	= ARRAY_SIZE(max17048_battery_props);
