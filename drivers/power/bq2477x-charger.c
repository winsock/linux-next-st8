                         ...
                         const struct i2c_device_id *id)
{
    ...
    chip_desc = devm_kzalloc(&bq2477x->dev, sizeof(*chip_desc), GFP_KERNEL);
    if (!chip_desc)
        return -ENOMEM;
    
    chip_desc->name	= "bq2477x-ac";
    chip_desc->type	= POWER_SUPPLY_TYPE_MAINS;
    chip_desc->get_property	= bq2477x_ac_get_property;
    chip_desc->properties		= bq2477x_psy_props;
    chip_desc->num_properties	= ARRAY_SIZE(bq2477x_psy_props);
    bq2477x->ac.desc = chip_desc;
    
    ret = power_supply_register(bq2477x->dev, bq2477x->ac.desc, &bq2477x->ac);
    if (ret < 0) {
        dev_err(bq2477x->dev,
