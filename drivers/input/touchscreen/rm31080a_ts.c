...
int g_service_bus_Y_report_count;
...
void raydium_change_scan_mode(u8 u8TouchCount)
{
    ...
    u16NTCountThd = (u16)g_st_ctrl.u8_time2idle * 100;
...
void raydium_report_pointer(void *p)
{
    ...
    if ((g_st_ctrl.u16_resolution_x != 0) && (g_st_ctrl.u16_resolution_y != 0)) {
        iMaxX = g_st_ctrl.u16_resolution_x;
        iMaxY = g_st_ctrl.u16_resolution_y;
    } else {
        iMaxX = RM_INPUT_RESOLUTION_X;
        iMaxY = RM_INPUT_RESOLUTION_Y;
    }
    
    iCount = max(ucLastTouchCount, spTP->uc_touch_count);
#ifndef INPUT_DEVICE_MULTIPLE_INSTANCES
    if (iCount && !spTP->uc_touch_count) {
        ucLastTouchCount = 0;
        ...
        for (i = 0; i < iCount; i++) {
            ...
            if ((spTP->uc_tool_type[i] == POINT_TYPE_ERASER)
                && (spTP->uc_slot[i] && INPUT_POINT_RESET)) {
                input_report_key(g_input_dev,
                                 BTN_TOOL_RUBBER, 0);
                input_mt_sync(g_input_dev);
                continue;
            ...
            if (i < spTP->uc_touch_count) {
                switch (spTP->uc_tool_type[i]) {
        ...
        input_sync(g_input_dev);
        ucLastTouchCount = spTP->uc_touch_count;
    }
#else
    if (iCount && !spTP->uc_touch_count) {
        ucLastTouchCount = 0;
    ...
    if (g_st_ctrl.u8_power_mode)
        raydium_change_scan_mode(spTP->uc_touch_count);
...
static struct rm_spi_ts_platform_data *rm_ts_parse_dt(struct device *dev, int irq)
{
            ...
            case KRL_CMD_CONFIG_CS:
                ...
                switch (pCmdTbl[_SUB_CMD]) {
                    case KRL_SUB_CMD_SET_CS_LOW:
                        g_spi->master->set_cs(g_spi, (bool)!!pCmdTbl[_DATA]);
                        break;
                }
                break;
            ...
            case KRL_CMD_READ_IMG:
                ...
                if (g_pu8BurstReadBuf != NULL) {
                    ret = rm_tch_spi_read(pCmdTbl[_ADDR],
                                          ...
                                          g_st_ctrl.u16_data_length);
                    if(g_st_ctrl.u8_ns_func_enable){
                        g_pu8BurstReadBuf[g_st_ctrl.u16_data_length] = g_stTs.bNsSel;
                        /*rm_printk("Raydium -Ns mode:%d,%x,%x\n",
                         g_st_ctrl.u16_data_length,g_pu8BurstReadBuf[g_st_ctrl.u16_data_length],g_stTs.bNsSel);*/
                    }
                }
...
static void *rm_tch_enqueue_start(void)
{
    ...
    if (!rm_tch_queue_is_full()) {
        g_service_bus_Y_report_count = 100 ;
        return &g_stQ.pQueue[g_stQ.u16Rear];
    }
    
    if (g_service_bus_Y_report_count < 0) {
        g_service_bus_Y_report_count = 100;
        rm_printk("Raydium - touch service is bus_Y,try again.\n");
    } else {
        g_service_bus_Y_report_count--;
    }
    return NULL;
}
...
static void rm_timer_work_handler(struct work_struct *work)
{
    ...
    if (g_stTs.u8WatchDogEnable) {
        ...
        if (u32TimerCnt > g_stTs.u32WatchDogTime &&
            g_stTs.u32WatchDogTime != g_st_ctrl.u8_watch_dog_normal_cnt) {
            g_stTs.u32WatchDogTime = g_st_ctrl.u8_watch_dog_normal_cnt;
            if (g_st_ctrl.u8_kernel_msg & DEBUG_DRIVER)
...
static irqreturn_t rm_tch_irq(int irq, void *handle)
{
    ...
    if (g_st_ctrl.u8_power_mode &&
        (g_stTs.u8ScanModeState == RM_SCAN_IDLE_MODE))
#ifndef INPUT_DEVICE_MULTIPLE_INSTANCES
        input_event(g_input_dev, EV_MSC, MSC_RAW, 1);
#else
    input_event(g_input_dev[0], EV_MSC, MSC_RAW, 1);
#endif
    mutex_unlock(&g_stTs.mutex_scan_mode);
    ...
    switch (index) {
        ...
        case RM_VARIABLE_REPEAT:
            ...
            if(g_st_ctrl.u8_ns_func_enable) {
                ...
                if(g_st_ctrl.u8_kernel_msg & DEBUG_DRIVER)
                    rm_printk("g_st_ctrl.bNsRpt %d\n",
                              g_stTs.bNsRpt);
            }
            break;
        case RM_VARIABLE_WATCHDOG_FLAG:
            rm_ctrl_watchdog_func(arg);
            g_st_ctrl.u8_ns_func_enable = ((u8)arg >> 7) & 0x01;
            if(g_st_ctrl.u8_kernel_msg & DEBUG_DRIVER)
                rm_printk("g_st_ctrl.u8_kernel_msg %d\n",
        ...
        case RM_VARIABLE_DPW:
            ...
            mutex_unlock(&g_stTs.mutex_ns_mode);
            if (g_st_ctrl.u8_kernel_msg & DEBUG_DRIVER)
                rm_printk("g_st_ctrl.DPW 0x%x:0x%x:0x%x # 0x%x:0x%x:0x%x\n",
                                    ...
                                    const struct rm_tch_bus_ops *bops)
{
    ...
    if (err) {
        dev_err(dev, "Raydium - irq %d bus_Y?\n", ts->irq);
        goto err_free_mem;
    }
...
struct rm_tch_ts *rm_tch_input_init(struct device *dev, unsigned int irq,
    ...
    for (index = 0 ; index < INPUT_DEVICE_AMOUNT ; index++) {
        ...
        if (index == INPUT_DEVICE_FOR_FINGER) {
            ...
            input_set_capability(ts->input[index], EV_KEY, BTN_TOUCH);
        }
